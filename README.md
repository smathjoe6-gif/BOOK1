...
videos to YouTube, TikTok, and X (Twitter), directly via their own APIs — no
Buffer, no subscriptions. Runs entirely on your own Mac.
...
and it gets copied into the other one too, so it reaches all 6 platforms
(YouTube, TikTok, X, Instagram, Facebook, Pinterest) no matter which folder
you use.
...
2. Create your real settings file:
   ...
   cp .env.example .env
   ```
   Open `.env` and fill in the Google, TikTok, and (optionally) X credentials
   (see your GK_Automation_Memory Google Doc). Never commit this file.
...
6. X (Twitter) needs no login step — just fill in `X_API_KEY`,
   `X_API_SECRET`, `X_ACCESS_TOKEN`, and `X_ACCESS_TOKEN_SECRET` in `.env`
   (copied straight from your app's "Keys and tokens" page at
   developer.x.com) and it works on the next restart. Leave `X_API_KEY`
   blank to skip X entirely.
...
there isn't), it posts to YouTube, TikTok, and X directly, then moves the
