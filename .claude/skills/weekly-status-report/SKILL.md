...
### Owen Scramble (the stronger, variance-theory scramble for digital nets)

Read `references/owen-scramble.md` for nested uniform scramble (NUS) — a fresh random permutation on each digit, keyed by the digits already chosen, giving stronger mixing than a digital shift while keeping a real QMC variance theory (`t` preserved in expectation, not pathwise). Covers the base-2 bit-flip-by-hash implementation for Sobol'/polynomial lattices, why a linear matrix scramble (LMS) is a cheaper-but-weaker relative and not Owen despite the name sometimes getting borrowed, why Halton needs a nested tree in each coordinate's own prime base rather than one base-2 tree copied across axes, and the chooser between digital shift, LMS, and Owen NUS depending on whether the point is cheap exactness or RMSE-via-independent-copies.
